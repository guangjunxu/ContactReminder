...
//	//============================所有处理类型数据库的方法start==============================
//	public static void loadType(MainActivity father)//从类型数据库中读取数据
//	{
//		try
//		{
...
//	//============================所有处理类型数据库的方法end==============================
	...
	//============================所有处理日程数据库的方法start==============================
	public static void loadPerson(MainActivity father)//从日程数据库读取日程数据
	{
		try
		{
			sld=SQLiteDatabase.openDatabase
			...
			String sql="create table if not exists person(" +
															...
															")";
			sld.execSQL(sql);
			Cursor cursor=sld.query("person", null, null, null, null, null, "nextdate desc");//按datetime1倒排序
		...
		{
			Toast.makeText(father, "Person数据库打开创建错误："+e.toString(), Toast.LENGTH_LONG).show();
			Log.d("exception",e.toString());
		}
	}
	
	public static void insertPerson(MainActivity father)//插入日程
	{
		try
		{
			sld=SQLiteDatabase.openDatabase
...
//	}
	...
	public static int getIdFromPrefs(MainActivity father)//读取preferences里面的日程sn
	{
